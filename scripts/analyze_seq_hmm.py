...
def _translate_non_stop(seq: str,  info: str) -> List[Tuple[str, str]]:
    """Translate the entire sequence, including stop codons, and attach metadata."""
    proteins = []
    for skew in [0, 1, 2]:
        protein = Seq(seq[skew:]).translate()
        proteins.append((protein, f'{info};skew_{skew};'))
    return proteins

def translate_sequence(seq: str, info: str) -> List[Tuple[str, str]]:
    """Translate a DNA sequence into a protein sequence and return it with metadata."""
    return _translate_non_stop(seq, info)
...
def analyze_sequences(hmms: List[pyhmmer.plan7.HMM], sequences: List[pyhmmer.easel.DigitalSequence], scores_global: np.ndarray, threads: int) -> np.ndarray:
    ...
    for hit in hits:
        for domain in hit:
            try:
                scores = np.array([(hit.query_name, domain.name, i.score, i.alignment.target_from, i.alignment.target_to, domain.evalue, i.i_evalue) for i in domain.domains])
                scores_global = np.concatenate([scores_global, scores])
            except ValueError:
                pass
    return scores_global

def analyze_file(seq_file: str, hmms: List[pyhmmer.plan7.HMM], batch_size: int, file_type: str, threads: int) -> pd.DataFrame:
    """Analyze sequences in a file using HMM profiles."""
    sequences = []
    scores_global = np.empty([0, 7])
    ...
    for i, dna_record in enumerate(SeqIO.parse(seq_file, file_type)):
        ...
        for seq, info in dna_seqs:
            translated = translate_sequence(seq, info)
            aa_seqs.extend(translated)


        # Convert protein sequences to pyhmmer-compatible format
        digitized_seqs = [pyhmmer.easel.TextSequence(sequence=str(seq), name=bytes(info, 'utf-8')).digitize(hmms[0].alphabet) for seq, info in aa_seqs]
    ...
    result_frame = pd.DataFrame(scores_global, columns=['Query', 'Name', 'Score', 'From', 'To', 'Hit_evalue', 'Domain_i_evalue'])
...
def main():
    ...
    for file in glob.glob(input_path):
        ...
        results[file] = analyze_file(file, hmms, args.batch, file_type, args.threads)
