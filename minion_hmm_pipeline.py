...
"""
USAGE
snakemake -s minion_hmm_pipeline.py --use-conda -j 30 -c 30 --config input_directory='TEST_INPUT/'  hmm_folder="MINION_HMM/" hmm_info='./hmm_info.csv' suffix='.fa' hmm_threads=10 output_directory="TEST_OUTPUT/" -k
"""
...
rule all:
    input:
        ...
        directory(os.path.join(OUTPUT_DIR, "ANNOTATED_CONTIGS"))
...
rule write_annotations:
    input:
        _sample_reports = expand(os.path.join(OUTPUT_DIR, "hmm_reports/{sample}.csv"), sample=SAMPLES)
    output:
        directory(os.path.join(OUTPUT_DIR, "ANNOTATED_CONTIGS"))

    shell:
        f"""
        python scripts/exclude_contigs.py {os.path.join(OUTPUT_DIR, 'hmm_reports')} {{output}}
        """
